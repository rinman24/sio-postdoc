...
class MmcrMergeRaw(TransformationStrategy):
    """Engine logic for raw Utqiagvik KAZR data."""
    ...
    def _add_variables(self, dataset: DataSet, path: Path) -> None:
        ...
        self._add_refl(dataset)
        self._add_spec_width(dataset)
        # self._add_mwr_lwp(dataset)  NOTE: You need to find this somewhere else.
    ...
    def _add_cloud_mask(self, dataset: DataSet) -> None:
        # Use reflectivity too create mask
        df = pd.DataFrame(
            dataset["Reflectivity"][:].data,
            index=dataset["time_offset"][:].data,
            columns=dataset["heights"][:].data,
        )
        # Create the mask
        df[df != DType.I2.min] = 1
        df.replace(DType.I2.min, 0, inplace=True)
        df.loc[:, df.columns < MIN_RANGE] = 0
        values = tuple(tuple(values) for values in df.values)
        # Add the variable
        self._variables["radar_mask"] = Variable(
            dtype=DType.I1,
            long_name="Radar Mask",
            scale=Scales.ONE,
            units=Units.NONE,
